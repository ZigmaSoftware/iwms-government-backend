...
class Municipality(BaseMaster):
    ...
    CACHE_SCOPES = ("municipality_list", "municipality_detail")
    ...
    @property
    def wards(self):
        """Wards under this municipality. Ward.municipality_id is a plain
        ...
        return Ward.objects.filter(municipality_id=self.unique_id)

    @property
    def customer_creations(self):
        """CustomerCreation rows scoped to this municipality. See `wards`
        above — CustomerCreation.municipality_id is likewise a plain
        unique_id string now."""
        from app.models.masters.customer_masters.customercreation import CustomerCreation

        return CustomerCreation.objects.filter(municipality_id=self.unique_id)
