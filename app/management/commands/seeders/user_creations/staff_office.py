...
class StaffOfficeSeeder(BaseSeeder):
    ...
    def run(self):
        ...
        for emp_name, username, dept_code, desig_name, district_name in self.STAFF:
            if StaffcreationOfficeDetails.objects.filter(username=username).exists():
                self.log(f"Staff '{username}' already exists — skipping.")
                continue
        ...
        self.log(f"---Staff office records seeded ({count} records)---")
